// see https://docs.mapbox.com/help/glossary/zoom-level/

export const matrix = {
  0: {
    0: 78271.484,
    20: 73551.136,
    40: 59959.436,
    60: 39135.742,
    80: 13591.701,
  },
  1: { 0: 39135.742, 20: 36775.568, 40: 29979.718, 60: 19567.871, 80: 6795.85 },
  2: { 0: 19567.871, 20: 18387.784, 40: 14989.859, 60: 9783.936, 80: 3397.925 },
  3: { 0: 9783.936, 20: 9193.892, 40: 7494.929, 60: 4891.968, 80: 1698.963 },
  4: { 0: 4891.968, 20: 4596.946, 40: 3747.465, 60: 2445.984, 80: 849.481 },
  5: { 0: 2445.984, 20: 2298.473, 40: 1873.732, 60: 1222.992, 80: 424.741 },
  6: { 0: 1222.992, 20: 1149.237, 40: 936.866, 60: 611.496, 80: 212.37 },
  7: { 0: 611.496, 20: 574.618, 40: 468.433, 60: 305.748, 80: 106.185 },
  8: { 0: 305.748, 20: 287.309, 40: 234.217, 60: 152.874, 80: 53.093 },
  9: { 0: 152.874, 20: 143.655, 40: 117.108, 60: 76.437, 80: 26.546 },
  10: { 0: 76.437, 20: 71.827, 40: 58.554, 60: 38.218, 80: 13.273 },
  11: { 0: 38.218, 20: 35.914, 40: 29.277, 60: 19.109, 80: 6.637 },
  12: { 0: 19.109, 20: 17.957, 40: 14.639, 60: 9.555, 80: 3.318 },
  13: { 0: 9.555, 20: 8.978, 40: 7.319, 60: 4.777, 80: 1.659 },
  14: { 0: 4.777, 20: 4.489, 40: 3.66, 60: 2.389, 80: 0.83 },
  15: { 0: 2.389, 20: 2.245, 40: 1.83, 60: 1.194, 80: 0.415 },
  16: { 0: 1.194, 20: 1.122, 40: 0.915, 60: 0.597, 80: 0.207 },
  17: { 0: 0.597, 20: 0.561, 40: 0.457, 60: 0.299, 80: 0.104 },
  18: { 0: 0.299, 20: 0.281, 40: 0.229, 60: 0.149, 80: 0.052 },
  19: { 0: 0.149, 20: 0.14, 40: 0.114, 60: 0.075, 80: 0.026 },
  20: { 0: 0.075, 20: 0.07, 40: 0.057, 60: 0.037, 80: 0.013 },
  21: { 0: 0.037, 20: 0.035, 40: 0.029, 60: 0.019, 80: 0.006 },
  22: { 0: 0.019, 20: 0.018, 40: 0.014, 60: 0.009, 80: 0.003 },
};

export function getMatrix(zoomLevel, latitude) {
  const zoomMatrix = matrix[zoomLevel];
  if (latitude > 80 || latitude < -80) {
    return zoomMatrix[80];
  } else if (latitude > 60 || latitude < -60) {
    return zoomMatrix[60];
  } else if (latitude > 40 || latitude < -40) {
    return zoomMatrix[40];
  } else if (latitude > 20 || latitude < -20) {
    return zoomMatrix[20];
  } else {
    return zoomMatrix[0];
  }
}

export function pixelsToMeters({ latitude, pixels, zoomLevel }) {
  const roundZoom = Math.round(zoomLevel);
  const ratio = zoomLevel === 0 ? 1 : roundZoom / zoomLevel;
  const latMatrix = getMatrix(roundZoom, latitude);
  return pixels * latMatrix * ratio;
}
